...
<head>
    ...
    if(isset($_POST['assign']))
    {
      if($_SERVER["REQUEST_METHOD"]=="POST")
      {
        ...
        $res1=mysql_query("SELECT p_id FROM police where p_name='$pname'",$conn);
        $q3=mysql_fetch_assoc($res1);
        $pid=$q3['p_id'];
      
      
        $res=mysql_query("update complaint set inc_status='Assigned',pol_status='In Process',p_id='$pid' where c_id='$cid'",$conn);
...
<select class="form-control" name="police_name" style="margin-left:40%; width:250px;">
            <?php
                        $p_name=mysql_query("select p_name from police where location='$location'");
                        while($row=mysql_fetch_array($p_name))
                        {
                            ?>
